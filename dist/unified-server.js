...
app.post('/api/v1/tools/execute-with-state', async (req, res) => {
    try {
        ...
        if (stateTrackingTools.includes(toolName)) {
            const { result, stateComparison } = await blockchainStateService.executeWithStateTracking(toolName, parameters, (tool, params) => zkPretUnifiedClient.executeTool(tool, params));
            res.json({
                ...result,
                stateComparison: {
                    ...stateComparison,
